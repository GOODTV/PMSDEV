...
public partial class View_ProgramEpisodeQuery : BasePage
{
    ...
    public void LoadFormData()
    {
        string strSql;
        DataTable dt;
        strSql = @"select  ProgramCode as 節目代號, [dbo].[getProgramName]([ProgramCode]) as 節目名稱, Episode as 集數 ,SubSeries as 系列名稱 ,EpisodeName as 分集名稱 ,PresenterSpeaker as 主持人or講員 ,
                    TotalLength as 總長度 ,Case when NoBroadcastNote = '1' then 'V' else '' end as 不播出註記 ,Case when NoReplayNote = '1' then 'V' else '' end as 不重播註記 ,Case when NoProvidevideoNote = '1' then 'V' else '' end as 不供片註記,ProgramID
        ...
        else
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                strBody += "<TR onclick =\"window.event.cancelBubble=true;window.open(\'" + Util.RedirectByTime("ProgramEpisodeEdit.aspx", "ProgramID=" + dr["ProgramID"].ToString()) + "\',\'_self\',\'\')\">";
                strBody += "<TD noWrap><SPAN>" + dr["節目代號"].ToString() + "</SPAN></TD>";
                strBody += "<TD><SPAN>" + dr["節目名稱"].ToString() + "</SPAN></TD>";
                strBody += "<TD noWrap><SPAN>" + dr["集數"].ToString() + "</SPAN></TD>";
                strBody += "<TD><SPAN>" + dr["系列名稱"].ToString() + "</SPAN></TD>";
                strBody += "<TD><SPAN>" + dr["分集名稱"].ToString() + "</SPAN></TD>";
                strBody += "<TD noWrap><SPAN>" + dr["主持人or講員"].ToString() + "</SPAN></TD>";
                strBody += "<TD noWrap><SPAN>" + dr["總長度"].ToString() + "</SPAN></TD>";
                strBody += "<TD noWrap><SPAN>" + dr["不播出註記"].ToString() + "</SPAN></TD>";
                strBody += "<TD noWrap><SPAN>" + dr["不重播註記"].ToString() + "</SPAN></TD>";
                strBody += "<TD noWrap><SPAN>" + dr["不供片註記"].ToString() + "</SPAN></TD></TR>";
            }
