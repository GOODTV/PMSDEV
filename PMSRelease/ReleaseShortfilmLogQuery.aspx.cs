...
public partial class View_ReleaseShortfilmLogQuery : BasePage
{
    ...
    public void LoadFormData1()
    {
        ...
        strSql = @"select R.SerNo,R.customerID as '客戶代號' , M.customerName as '客戶名稱' ,CONVERT(VarChar,[supplyDate],111) as '供片年月'
                    , R.filmID as '短片代號', [dbo].[getProgramName](R.filmID) as '短片名稱', CF.Episode as '集數', filename as '檔名'
                    from dbo.ReleaseShortFilmLog R
                    left join [dbo].[_CF02P0] CF on CF.CFID=R.filmID
        ...
        {
            strSql += " and CF.Episode >= " + tbxEpisodeNo_Start.Text.Trim() + " ";
        }
        if (tbxEpisodeNo_End.Text.Trim() != "")
        {
            strSql += " and CF.Episode <= " + tbxEpisodeNo_End.Text.Trim() + " ";
        }
        ...
        strSql += " order by R.filmID,R.customerID,cast(CF.Episode as integer),R.SerNo ";
    ...
    public void LoadFormData2()
    {
        string strSql;
        DataTable dt;
        strSql = @"select R.SerNo,R.filmID as '短片代號', [dbo].[getProgramName](R.filmID) as '短片名稱', CF.Episode as '集數', R.customerID as '客戶代號' , M.customerName as '客戶名稱' 
        ...
        strSql += " order by R.filmID,cast(CF.Episode as integer),R.customerID,R.SerNo ";
        HFD_Key.Value = tbxCustomerID.Text + ";" + tbxFilmID.Text + ";" + tbxEpisodeNo_Start.Text + ";" + tbxEpisodeNo_End.Text + ";" + ddlYearS.SelectedValue + ";" + ddlMonthS.SelectedValue + ";" + ddlYearE.SelectedValue + ";" + ddlMonthE.SelectedValue;
        dt = NpoDB.GetDataTableS(strSql, dict);
        GridView1.CssClass = "table-list";
        GridView1.DataSource = dt;
