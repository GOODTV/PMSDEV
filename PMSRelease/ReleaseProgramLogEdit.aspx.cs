...
public partial class View_ReleaseProgramLogEdit : BasePage
{
    ...
    {
        bool flag = false;
        //依節目代號及集數檢核分集資料庫中不供片標記
        string strSql = "";

        DataTable dt = null;
        Dictionary<string, object> dict_check = new Dictionary<string, object>();
        strSql = "select ISNULL(NoProvidevideoNote,'') as NoProvidevideoNote from [dbo].[ProgramEpisode] where ProgramCode='" + txtProgramID.Text.Trim() + "' and Episode='" + txtEpisode.Text.Trim() + "'";
        ...
        if (dt.Rows.Count != 0)
        {
            ...
            {
                this.Page.RegisterStartupScript("s", "<script>alert('此集不供片，請聯絡片庫主管！');</script>");
                return;
            }
        }
        //檢核是否有此客戶
    ...
    public void ReleaseProgramLog_Edit()
    {
        ...
        dict.Add("ModifyUser", SessionInfo.UserID);
