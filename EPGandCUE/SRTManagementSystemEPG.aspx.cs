...
public partial class SRTManagementSystemEPG : BasePage
{
    ...
    protected void btnEPG_Click(object sender, EventArgs e)
    {
        // 導向EPG匯入頁面
        Response.Redirect("UploadEPG.aspx");
    }
    #endregion
    ...
    */
    ...
    protected void GridDataBind()
    {
        ...
        // 開始日期
        string startDate = this.StartDate_box.Text;    
        // 結束日期
        string endDate = this.EndDate_box.Text;
        // 頻道別
        ...
        if (startDate != "" && endDate != "")
        {
            ...
            if (ts.Days > 30) msg += "開始日期至結束日期的範圍太大！請選擇一個月內範圍。\\n";
        }
        if (msg != "")
        {
        ...
        */
        ...
        DataTable dt = objEpgService.GetEpgList(startDate, endDate, channel, planningTitle, episodeNumber, houseNo, premier);
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                {
                    if (!string.IsNullOrEmpty(dt.Rows[i]["Subtitle"].ToString()) && !string.IsNullOrEmpty(dt.Rows[i]["SubtitleSRT"].ToString()))
                    {
                        //TXT 有 SRT 有
                        //修改為24小時制
                        dt.Rows[i]["UploadRecord"] = ((System.DateTime)dt.Rows[i]["ModifiedDatetime"]).ToString("yyyy/MM/dd HH:mm:ss");
                    }
                    else if (string.IsNullOrEmpty(dt.Rows[i]["Subtitle"].ToString()) && !string.IsNullOrEmpty(dt.Rows[i]["SubtitleSRT"].ToString()))
                    {
                        //TXT 沒有 SRT 有
