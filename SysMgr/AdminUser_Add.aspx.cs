    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        if (txtPassword.Text == txtRePassword.Text && txtPassword.Text != "")
        {
            if (txtPwdValidDate.Text == "" || Convert.ToDateTime(txtPwdValidDate.Text) >= System.DateTime.Now)
            {
                ...
                strSql += " (UserID, OrgID, DeptID, GroupID, Password, PwdValidDate, UserName, CreateDate, Email, MobilNo, Memo, IsUse) values";
                strSql += "(@UserID,@OrgID,@DeptID,@GroupID,@Password,@PwdValidDate,@UserName, GetDate(), @Email,@MobilNo,@Memo,@IsUse); ";
                strSql += "\n";
